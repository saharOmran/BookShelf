...
KAFKA_TOPIC = os.getenv("KAFKA_TOPIC", "book-events")
...
IAM_SERVICE_URL = "http://localhost:8000/token"
...
async def add_book(
    ...
    image_url = f"http://localhost:8002/uploaded_files/{file_id}"
    ...
    try:
        ...
        producer.produce(KAFKA_TOPIC, key="book-added", value=json.dumps(book_data))
        producer.flush()
...
async def edit_book(
    ...
    try:
        ...
        if image:
            content = await image.read()
            file_id = fs.put(content, filename=image.filename)
            existing_book["image_url"] = f"http://localhost:8002/uploaded_files/{file_id}"
...
async def upload_file(file: UploadFile = File(...)):
    ...
    file_url = urljoin("http://localhost:8002/uploaded_files/", str(file_id))
